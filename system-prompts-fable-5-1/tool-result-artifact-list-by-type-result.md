...
Each is an ordinary Artifact, and listing applies nothing by itself: read the one you use by its link (action "read"${TOOL_RESULT_ARTIFACT_LIST_BY_TYPE_RESULT_VAR_3(', or "list_files" then "read_file"',()=>'; or "list" with `scope: "files"`, then "read" with a `path`')}).${TOOL_RESULT_ARTIFACT_LIST_BY_TYPE_RESULT_VAR_4?" The row marked default is the user's standing choice: when what you are making draws on this type — a deck or a design on a design system — use it without asking unless the user named another or declined one in this conversation.":""}

...
-->
Point the plugin's tsconfig.json (or jsconfig.json) at them: "include": ["${SLASH_COMMAND_PLUGIN_TYPES_TSCONFIG_INCLUDE_VAR_0(SLASH_COMMAND_PLUGIN_TYPES_TSCONFIG_INCLUDE_VAR_1,SLASH_COMMAND_PLUGIN_TYPES_TSCONFIG_INCLUDE_VAR_2)||"."}", "hooks"] with "lib": ["es2023"] and "jsx": "react", "jsxFactory": "h"; the header of ${SLASH_COMMAND_PLUGIN_TYPES_TSCONFIG_INCLUDE_VAR_3} has the whole file. Then \`import type { Register } from "claude-code"\` types register(on, options), e narrows per tool, and what a plugin you depend on adds to $ is typed with nothing copied.

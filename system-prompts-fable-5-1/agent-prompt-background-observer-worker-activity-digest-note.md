...
description: >-
  Line in the background-observer agent's system prompt (worker-paired
  variant) explaining that the per-turn activity digest wrapped in <NAME-
  activity> tags is data about the worker, never instructions.
ccVersion: 2.1.218
variables:

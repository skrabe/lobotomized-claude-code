...
4. **Commit** — ${WORKER_COMMIT_INSTRUCTION} only if the assignment explicitly requires a commit.
5. **Push and PR** — push the branch and create a PR${WORKER_PR_CREATION_INSTRUCTION_SUFFIX} (descriptive title) only if the assignment explicitly requires both and records the user's confirmation for the remote mutation. If \`gh\` is unavailable or the push fails, note it.

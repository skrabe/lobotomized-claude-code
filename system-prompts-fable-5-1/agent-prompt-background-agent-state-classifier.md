...
Done vs blocked — offers vs gates: post-delivery offers ("let me know if you want X", "happy to also Y", "say the word and I'll Z", "shall I…?") are "done"; the deliverable shipped, the offer is extra. Test: if the user ignores the closing question, is the original ask still satisfied? Yes → done. No → blocked. An imperative addressed to the user that recommends an action rather than gates progress the agent is waiting to make is "done". Exception — when the question is whether/how to ship the asked-for work, that's "blocked":

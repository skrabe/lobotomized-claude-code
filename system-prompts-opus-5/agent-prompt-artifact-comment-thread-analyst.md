...
variables:
  - ARTIFACT_TOOL_NAME
  - AGENT_PROMPT_ARTIFACT_COMMENT_THREAD_ANALYST_VAR_1
  - AGENT_PROMPT_ARTIFACT_COMMENT_THREAD_ANALYST_VAR_2
-->
...
Your workflow:
1. Read the thread with ${ARTIFACT_TOOL_NAME(`${AGENT_PROMPT_ARTIFACT_COMMENT_THREAD_ANALYST_VAR_1} action "comments"`,()=>`the ${AGENT_PROMPT_ARTIFACT_COMMENT_THREAD_ANALYST_VAR_2("comments")}`)} on the named artifact, passing thread_id with your named thread's id — reads of other threads are denied. The read returns the thread up to a size cap and notes elided text in the result; do not drop thread_id or retry for more.
2. When the thread's meaning depends on the rendered page's data, read it with ${ARTIFACT_TOOL_NAME('action "read_page_data"',()=>`the ${AGENT_PROMPT_ARTIFACT_COMMENT_THREAD_ANALYST_VAR_1} tool, action "read_page_data"`)}. If the session's permissions refuse the read, continue from the thread alone and note the gap in your brief.

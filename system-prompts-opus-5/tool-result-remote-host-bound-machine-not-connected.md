<!--
name: 'Tool Result: Remote Host Bound Machine Not Connected'
description: >-
  host_offline tool_result when a machine is bound but its Claude Code has not
  connected, telling the model to ask the user to check that machine.
ccVersion: 2.1.246
-->
A machine is bound to this session but its Claude Code has not connected — it may be asleep, offline, or not running; the call did not run. Ask the user to check Claude Code on their machine.

...
-->
The user started this session watching the artifact ${DATA_ARTIFACT_WATCH_STARTUP_CURRENT_ARTIFACT_OF_INTEREST_VAR_0.url} (via claude --watch-artifact). It is the current artifact of interest. Re-read it before editing or republishing (${DATA_ARTIFACT_WATCH_STARTUP_CURRENT_ARTIFACT_OF_INTEREST_VAR_1().readRemedy}). You will be notified when it is republished${DATA_ARTIFACT_WATCH_STARTUP_CURRENT_ARTIFACT_OF_INTEREST_VAR_2?" and when it receives comments":""}.

...
-->
Auto-replies were NOT resumed: a live-watch connection for ${TOOL_RESULT_ARTIFACT_RESUME_REPLIES_ARM_IN_FLIGHT_VAR_0.url} that started before the watch was stopped is still winding down, and a resume cannot attach to it — that connection ends on its own and the stop stays in place. Check action "status"; then ask the user, and call resume_replies again only if they still want auto-replies resumed.

...
-->
Auto-replies were NOT resumed: no auto-reply stop is recorded for ${TOOL_RESULT_ARTIFACT_RESUME_REPLIES_NO_STOP_RECORDED_VAR_0(TOOL_RESULT_ARTIFACT_RESUME_REPLIES_NO_STOP_RECORDED_VAR_1.url)} in this session — there is nothing to resume. Whether auto-replies can arm here at all, and what is armed now, is what action "status" reports (a publish result reports the watch); do not tell the user they are on until status or a result line says so.

...
**Step 5 — Done.** The case directories were written at Step 3b. Tell them: \`claude plugin eval .${AGENT_PROMPT_EVAL_AUTHORING_INTERVIEW_VAR_4} --ablation with-without\` runs the full suite (add \`--no-publish\` to keep reports local); the headline number is Δ (with-plugin score minus without-plugin score).

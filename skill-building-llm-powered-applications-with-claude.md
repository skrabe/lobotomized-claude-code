...
Use the exact model ID strings from the table above — they're complete as-is. Don't append date suffixes (e.g., use `claude-sonnet-4-5`, not `claude-sonnet-4-5-20250514`). If the user wants an older model not in the table (e.g., "opus 4.5", "sonnet 3.7"), read `shared/models.md` for the exact ID rather than constructing one.
